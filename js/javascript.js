...
var windFactor = ((Math.random()*2)-2);
function windGenerator() {
document.getElementById("theWind").innerHTML = "" + windFactor;

}
setInterval(function() { windFactor = ((Math.random()*2)-2);
...
takes in smoke color(Not used), windFactor (generated by windGenerator())
and the canvas var which it should draw to */
function drawSmoke(smokeColor, canvasVar) {
	function smokeParticle() {
		...
		var prt = setInterval(function () {
			if (y > -100) {
				...
				canvasVar.clearRect(x-radius-windFactor,y-radius,3*radius,2.5*radius);
				//Smoke particle gradient that slows fades
				var smokeGrd = canvasVar.createRadialGradient(x,y,0,x,y,radius);
				gradientBuilder = gradientBuilder + gradient1 + ")"
				smokeGrd.addColorStop(0,gradientBuilder);
				gradient1 -= 0.008;
				gradientBuilder = gradientBuilder + gradient2 + ")"
				gradient2 -= 0.008;
				smokeGrd.addColorStop(0.4, gradientBuilder);
				...
				y -= yDecrementFactor;
				//yDecrementFactor *= 0.999;
			} else {
				clearInterval(prt);
			}
		}, 50); }
	setInterval(smokeParticle, ((Math.random()) * 2000)+2000);
	}

function drawSmoke1() {
	var smoke = document.getElementById("mySmoke1");
	var mySmoke = smoke.getContext("2d");
	drawSmoke("lol", mySmoke);
}
function drawSmoke2() {
